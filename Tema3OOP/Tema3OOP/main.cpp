...
template<typename T>
void listStorageItems(const Storage<T>& storage) {
...
static void readProductsFromFile(const std::string& filename, std::vector<std::shared_ptr<Product>>& products) {
    ...
    while (std::getline(file, line)) {
        ...
        std::string type;
        getline(iss, type, ','); // Read the type of the product
...
int main() {
    Cart cart;
    Storage<std::shared_ptr<Product>> productsStorage;
    ...
    do {
        try {
            ...
            std::cout << "11. Exit\n";
            std::cout << "12. List storage items\n";
            ...
            case 1:
            case 2: {
                std::string type = (choice == 1 ? "Electronics" : "Clothing");
                std::shared_ptr<Product> product = factory.createProduct(type);
                if (product) {
                    product->input(std::cin);
                    productsStorage.addItem(product);
                }
                break;
            }
            case 3: {
            ...
            case 11:
                ...
                break;
            case 12:
                listStorageItems(productsStorage);
                break;
            default:
